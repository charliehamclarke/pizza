...
def main():
    ...
    for i, burger_id in enumerate(burger_order):
        burger_type = burger_menu.loc[burger_menu['Num ID'] == burger_id, 'Type'].values[0]
        size = burger_order_size[i]
        
        if size == "small":
            cost_food += burger_menu.loc[burger_menu['Num ID'] == burger_id, 'Small'].values[0]
        elif size == "medium":
            cost_food += burger_menu.loc[burger_menu['Num ID'] == burger_id, 'Medium'].values[0]
        elif size == "large":
            cost_food += burger_menu.loc[burger_menu['Num ID'] == burger_id, 'Large'].values[0]
